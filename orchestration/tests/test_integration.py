...
from helpers import config_for, initialize_repository, initialize_separate_repository


class IntegrationTests(unittest.TestCase):
    def test_verified_candidate_is_staged_then_fast_forwarded(self) -> None:
        for layout, initializer in (
            ("standard", initialize_repository),
            ("separate-git-dir", initialize_separate_repository),
        ):
            with self.subTest(layout=layout), tempfile.TemporaryDirectory() as temporary:
                self.exercise_integration(Path(temporary), initializer)

    def exercise_integration(self, root: Path, initializer: object) -> None:
        if not callable(initializer):
            self.fail("repository initializer is not callable")
        base = initializer(root)
